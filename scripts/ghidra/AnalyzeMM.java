// Consolidates: DumpMCCAMLoader, DumpMMCAMMission, DumpMMLayerSlot,
//               DumpMMLayerSlot2, DumpMMLayerSlot3, DumpMMLayerSlot4

public class AnalyzeMM extends FAScript {

    @Override
    public void run() throws Exception {
        openOutput("AnalyzeMM");

        // MM keyword handler
        header("MM keyword handler FUN_0047a510 (0x47a510)");
        dumpAt(0x0047a510L);

        header("Callers of FUN_0047a510");
        dumpCallers(0x0047a510L);

        // MM line parser
        header("FUN_0047a130 (MM line parser)");
        dumpAt(0x0047a130L);

        header("Callers of FUN_0047a130");
        dumpCallers(0x0047a130L);

        // Mission init
        header("_MISSIONInit2 variant 1 (0x480b50)");
        dumpAt(0x00480b50L);

        header("_MISSIONInit2 variant 2 (0x480a30)");
        dumpAt(0x00480a30L);

        header("Callers of _MISSIONInit2 (0x480b50)");
        dumpCallers(0x00480b50L);

        // MC/Campaign loader
        header("MC loader (0x481940)");
        dumpAt(0x00481940L);

        header("Callers of MC loader");
        dumpCallers(0x00481940L);

        header("Pre-MC (0x480750)");
        dumpAt(0x00480750L);

        header("Campaign launcher (0x428412)");
        dumpAt(0x00428412L);

        header("FUN_00481c10");
        dumpAt(0x00481c10L);

        // Layer slot
        header("ParseLayerFile (0x4b4370)");
        dumpAt(0x004b4370L);

        header("FUN_004b3480");
        dumpAt(0x004b3480L);

        header("GetLayerByIndex (0x4b3170)");
        dumpAt(0x004b3170L);

        header("Callers of GetLayerByIndex (0x4b3170)");
        dumpCallers(0x004b3170L);

        // MM lib area
        header("FUN_0044f180");
        dumpAt(0x0044f180L);

        header("FUN_00430a90");
        dumpAt(0x00430a90L);

        header("FUN_0043a5c0");
        dumpAt(0x0043a5c0L);

        // Keyword helpers
        header("Keyword func 0x4ace50");
        dumpAt(0x004ace50L);

        header("Keyword func 0x4acfa0");
        dumpAt(0x004acfa0L);

        // MC/CAM range
        header("All functions 0x481800-0x482200");
        dumpRange(0x00481800L, 0x00482200L);

        // String search
        header("Strings: MM keywords and file extensions");
        searchStrings(new String[]{"textFormat", "map", ".T2", ".LAY", "LAY ", "tmap", "tdic",
                "mission", ".MM", "theater"});

        // Symbol search
        header("Symbols matching mm/map/mission/campaign/cam/mc/theater");
        dumpSymbolsMatching("mm", "map", "mission", "campaign", "cam", "theater",
                "mcload", "mapload", "mapinit");

        closeOutput();
    }
}
