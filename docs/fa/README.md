...
The RE effort has covered all 44 known binary and text formats, the full FA.EXE symbol map (3,829 recovered C++ symbols), and the major runtime subsystems â€” flight model, renderer, AI, networking, and the Win32 overlay DLL architecture. The `fx_lib` codecs and `fx` CLI commands exist to validate the format specs: a working, byte-identical round-trip implementation is the proof that a format is fully understood. Cross-references between the docs and the tools are noted throughout.
...
| [modding.md](modding.md) | Step-by-step modding recipes â€” textures, stats, missions, audio, and more |
