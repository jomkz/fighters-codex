...
- **CN_INFO `[0xc0]`–`[0xcf]` and `[0x5d0]`–`[0x8e3]` residual gaps (~16 + ~788 bytes)**: AnalyzeNetwork RE (2026-05-20) confirmed MODEM.DAT and SERIAL.DAT share the CN_INFO struct. Phone book layout resolved: `[0xd0]`–`[0x34f]` = 8 × 0x50-byte player names; `[0x350]`–`[0x5cf]` = 8 × 0x50-byte phone numbers (both MODEM.DAT only, zeroed in NET.DAT). Remaining unknowns: `[0xc0]`–`[0xcf]` (16-byte header/count before phone book) and `[0x5d0]`–`[0x8e3]` (~788 bytes after phone book — possibly more modem init strings or serial data). *Approach: MODEM.DAT differential save — add/remove phone book entries, compare bytes in those two sub-ranges.* See [formats/NET.md](formats/NET.md).
