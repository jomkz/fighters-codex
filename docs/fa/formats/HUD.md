...
| 7 | `0x00080` | `FUN_00407930`; set/cleared by `FMBrakes` (flap actuator) via input 0x62 | Advisory icon B active â€” flap deployed |
| 8 | `0x00100` | `FUN_00407930`; set/cleared by `FMFlaps` (gear actuator) via input 0x66 | Advisory icon A active â€” landing gear deployed (down) |
| 9 | `0x00200` | `FUN_00407930`; set/cleared by `FMBay` (tailhook actuator) via input 0x6f | Advisory icon D active (single-player path) â€” tailhook deployed |
| 10 | `0x00400` | `FUN_00407930`; set/cleared by `FMHook` (bay-door actuator) via input 0x68 | Advisory icon D active (multiplayer path, also requires `DAT_0050d322 & 2`) â€” weapon bay door / hook variant open |
| 11 | `0x00800` | `HARDSetFlags` (weapon-state scan, each tick) | Active weapon lock â€” at least one weapon has ammo and an acquired lock |
| 12 | `0x01000` | `FUN_00407a00`; toggled by `SetAutopilot` via input 0x61 | Flight-lock / autopilot active â€” replaces throttle/G readout with lock sprite (`DAT_004ebf94`) |
| 13 | `0x02000` | `FlightKey` case 0x61 (autopilot key handler) | Autopilot ILS/ACLS sub-mode â€” set alongside bit 12 when flight mode is 6 and aircraft has ACLS capability (PT+0xe9 â‰  0); gates carrier-approach glide-slope computation |
| 14 | `0x04000` | `?MPReceive@@YGDXZ` (0x46C980 = FUN_0046c98f, decompile failed â€” writes at 0x46db2b) in multiplayer; unanalyzed code at 0x4bc177/0x4bc190 may also write it in single-player. No function in FA.EXE was found to set this bit via a direct `OR [mem], 0x4000` constant â€” the SP writer path remains unknown. Read in `ServicePlayer` during ejection states 0x11/0x12 in conjunction with `DAT_0050d0b1` (nearest entity pointer) and entity+0xFB range comparison; also gates aerodynamic integrator reset (`stickX`/`ec`, `rudder`). | Unknown â€” likely a network-synced or proximity-alert advisory state |
| 15 | `0x08000` | `PLANECheckFuel` via `FMUpdatePlaneFields` (fuel monitor, runs every 5 ticks) | **Bingo fuel** threshold reached â€” `@SAYLowFuelMessage@8` checks `(0x8000 set) && (0x80000 clear)` â†’ plays "Bingo fuel" voice line, then sets bits 19â€“20 as inhibit |
| 16 | `0x10000` | `PLANECheckFuel` via `FMUpdatePlaneFields` | **Joker fuel** threshold reached â€” checked `(0x10000 set)` â†’ plays "Joker fuel" voice line, sets bit 20 inhibit |
| 17 | `0x20000` | `PLANECheckFuel` via `FMUpdatePlaneFields` | **Running on fumes** threshold reached â€” `(0x20000 set) && (0x200000 clear)` â†’ plays "Running on fumes" voice line, sets bits 19â€“21 inhibit |
| 18 | `0x40000` | `PLANECheckFuel` via `FMUpdatePlaneFields` | **Out of fuel** threshold reached â€” `(0x40000 set) && (0x400000 clear)` â†’ plays "We're out of gas / I'm out of fuel" voice line, sets bits 19â€“22 inhibit |
...
| 28 | `0x10000000` | `HUDDrawSpeed`, `HUDDrawAlt`; set by `_DAMAGEDoHit@12` state `0x50d40e` | Classified / redacted display â€” speed tape substitutes string at `0x004ebfbc`; altitude tape substitutes `s_XXXXX_004ebfd8` ("XXXXX"); tape tick-mark rendering skipped entirely; also triggered by aircraft-out-of-control damage state |
| 29 | `0x20000000` | `_DAMAGEDoHit@12` damage state `0x50d410` | Emergency state â€” aircraft critical / imminent crash indicator |
| 30 | `0x40000000` | `_DAMAGEDoHit@12` damage state `0x50d40f` (conditional on `Rand(3)` result); also read by carrier HGR renderer (`AnalyzeHGR.txt` lines 4143/4315): when set, suppresses normal slot-dot rendering and fixes the approach-angle indicator at position 2 (of 0â€“9) instead of computing the live angle from `DAT_0050ce9f` | Emergency state variant A â€” aircraft spinning / uncontrolled flight |
| 31 | `0x80000000` | `_DAMAGEDoHit@12` damage state `0x50d40f` (conditional on `Rand(3)` result) | Emergency state variant B â€” aircraft spinning / uncontrolled flight (alternate roll) |
...
|------|-----|---------------|-----------------|------------------|-----------|
| A | `0x100` | `+0x245` | `GEAR` | `GEAR` | `FMFlaps` â€” gear actuator (input 0x66) |
| B | `0x080` | `+0x24D` | `FLAP` | `FLAP` | `FMBrakes` â€” flap actuator (input 0x62) |
| C | `0x040` | `+0x255` | `BRAKE` | `BRAKE` | `FMGear` â€” speedbrake actuator (input 0x67) |
...
- New `lib/src/hud.cpp` + `lib/include/fx/hud.h` â€” parse sprite name table and gauge parameter block
- New `cli/cmd_hud.cpp` â€” `fx hud dump <file.HUD>` prints gauge table as JSON `[{gauge, dx, dy}]`
