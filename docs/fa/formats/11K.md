...
`fx lib unpack` maps `&` and `^` to `_` on extraction because Windows rejects those
characters in filenames. The original names are preserved in memory for patching.

## PCM Encoding

- Signed 8-bit (`int8_t`), range âˆ’128..127
- Mono (1 channel)
- No header, no footer â€” the file is raw samples from byte 0
...
fx audio unpack <file.11K|.5K|.8K> [-o out.wav] # raw PCM â†’ WAV
fx audio pack   <in.wav> -o <out.11K|.5K|.8K>   # WAV â†’ raw PCM
...
1. Prepare a mono 8-bit 11,025 Hz WAV and pack it to `.11K` with `fx audio pack`.
...
To create a `.5K` variant: pack at 11025 Hz after doubling playback speed 2Ã—, then
...
_AIL_unlock_0();
_AIL_register_timer_4(&_PollMod__YGXK_Z) â†’ _timerHandle;    // register poll callback
...
_AIL_allocate_sample_handle_4(_soundDriverHandle) â†’ _mixChanHandle[i]
```
...
`_AIL_end_sample_4(channel)` â€” called per channel on shutdown.
...
Use `fx audio unpack` to convert to WAV, edit, then `fx audio pack` to re-encode.
Audacity can also import the raw PCM file directly without the `fx` step (*File â†’
Import â†’ Raw Data*: signed 8-bit, mono, sample rate from extension).

- **Audacity** â€” free, cross-platform; raw import, noise reduction, pitch/tempo tools
- **Adobe Audition** `$` â€” paid; professional mastering and spectral repair
