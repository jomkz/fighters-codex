...
|-----------|-------------|-------|
| `.5K` | 5512 Hz | Half of `.11K`; the extension is a rounded label. **Not 5000** — see below |
...
The rate is **not stored in the file** — it comes from the extension — so no round-trip
can contradict it, and `lib/src/audio.cpp` returned 5000 Hz for `.5K` while this table
said 5512. Every `.5K` the tool emitted played **10% long**, and the unit test asserted the
wrong value, so the suite stayed green ([#491](https://github.com/jomkz/fighters-codex/issues/491)).

The **briefing videos are an independent clock** that settles it. Audio is shared per
3-character group ([VDO.md](VDO.md)), the paired `.FBC` holds one `u32` per frame, and the
`.VDO` header gives the fps — none of which the audio codec has any say in:

| | bytes | at 5000 Hz | at 5512 Hz | video (1,685 frames ÷ 15 fps) |
|---|---|---|---|---|
| `IQC.5K` | 619,237 | 123.85 s (**+10.2%**) | 112.34 s (**+0.0%**) | **112.33 s** |

`tests/test_audio.cpp` now checks this against the whole install (106 tracks): a narration
may end before its clip does, but it may never outrun it.
...
| `#` | Radio speech phrase — a second voice family beside `^`; `.5K` only in the retail corpus (×85 in FA_2, many stems shared with `^`) | `#AIRBORN.5K`, `#ALMSTHM.5K` |

`fx lib unpack` maps only `&` to `_` on extraction (via `ealib_safe_name`, which
remaps `& * ? " < > | / \ :` for cross-platform legality); `^` is a legal filename
character and extracts with the caret intact (e.g. `^ENGAGE.11K`), as does `#`. See
[LIB.md](LIB.md#filename-conventions). The original names are preserved in memory for
patching.
...
[VDO](VDO.md) — briefing videos share one `.11K` narration per 3-character
