...
Win32 PE DLL. All observed `.MUS` files decompress to **4608 bytes**. String analysis of `M_AIR.MUS` yields only the standard PE header strings â€” no embedded `.XMI` track names are visible as plain text, confirming XMI references are encoded as integer indices resolved at runtime.
...
The index space is **sparse** â€” many slots have no corresponding file in FA_2.LIB (e.g. indices 2, 8, 10â€“12, 15, 17, 20, 27, 29, 30, 32â€“37, etc.). The numeric suffix in the filename IS the track index; `VALK01.XMI` is the sole exception to the `AIRnnn` naming pattern and occupies index 1.
...
| `FB <mode> <idx>` | 3 bytes | Play XMI track â€” short form (no `F9` terminator); appears in M_LAUNCH context |
...
The `01 02 03 02 01 02 03 02 01` byte pattern immediately following `FC` is a **state machine dispatch table** â€” the same pattern appears in DLG CODE sections just before JMP thunks, identifying it as a shared engine construct.
...
| `M_EJECT.MUS` | `"air"` | *(none)* | No `FB` opcodes; contains only `FD`/`FE` control flow â€” eject event redirects state rather than starting a new track |
...
Setup opcodes:
- `FA 21 0x48` â€” likely fade-in time
- `FA 21 0x7E` â€” likely fade-out time
- `FA 32 0x30` â€” likely tempo/volume
...
_SEQmusic(name, seq_idx)
  ...
  â†’ calls MusicOn(filename, seq_idx)
      â†’ RMAccess(filename, 0x10c)   â€” loads MUS DLL from LIB archive
      â†’ _AIL_allocate_sequence_handle   â€” allocate Miles Sound System handle
      â†’ _AIL_init_sequence(handle, mus_data, seq_idx)  â€” pass MUS CODE section to AIL
      â†’ _AIL_start_sequence(handle)     â€” begin playback
```

**The MUS CODE section is passed directly to the Miles Sound System (AIL).** FA does not interpret the FA/FB/FC/FD/FE bytes itself â€” Miles processes them natively as XMIDI or MSS sequence data. The sub-opcode semantics (`FA 0x19`, `FA 0x21`, etc.) are Miles-internal and cannot be decoded from FA.EXE alone.

### Volume

`?MusicVolume(vol)` maps the 0â€“100 game volume scale to AIL's 0â€“127 range:
```
...
The `short param_2` passed through `_SEQmusic` â†’ `MusicOn` â†’ `_AIL_init_sequence` is the **AIL sequence index** â€” which section of the XMIDI data to start playback from. Normally 0 (first sequence).

## Toolkit Roadmap

- New `cli/cmd_mus.cpp` â€” `fx mus dump <file.MUS>` prints the raw opcode stream and resolves `FB <idx>` values to XMI filenames using the index rule above
- No lib codec needed â€” MUS is passed to AIL as-is; the dump walks the byte stream

## Related

- [XMI.md](XMI.md) â€” Extended MIDI audio tracks played by the music system
- [SEQ.md](SEQ.md) â€” cutscene sequencer, which may also trigger music state changes
