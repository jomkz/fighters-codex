...
```
ft mission info   <file.M>              # map name, time, object count
ft mission unpack <file.M> [-o out.txt] # editable text
...
`.M` files require `ft mission unpack` → edit → `ft mission pack`. `.MT` files
