...
- **LAYER array** — `N` entries Ã— 0x160 bytes each, at a CODE VA stored in the header. Each entry defines one sky condition (altitude range, gradient table, cloud textures, visibility).
...
| +0x02 | s32 | **sel_alt_min** — **raw** altitude lower bound; `CopyLayersToRuntime` selects this layer when `sel_alt_min â‰¤ currentTimeOfDay â‰¤ sel_alt_max` |
...
| +0x16 | s32 | **vis_lo** — fog visibility at `fog_alt_low` (0x00..0xFF; scaled to colour_array index as `vis_lo Ã— count >> 8`) |
...
| +0x3E | u8[93] | **zenith_grad** — 31 Ã— 3-byte RGB entries from zenith toward horizon (indices 0–30) |
| +0x9B | u8[96] | **horizon_grad** — 32 Ã— 3-byte RGB entries from horizon downward (indices 0–31) |
...
| +0xFE | u32 | **fog_density** — per-frame fog opacity; updated each frame by `WRFogLayerUpdate` (random jitter Â±25, clamped to [0xD9, 0xEB] = [217, 235]) |
...
| +0x34 | DAT_00580de4 | **sky_layer_array[7]** — indices 7–9 are the array's high-angle tail; reachable when `(angle Ã— sky_angle_scale) >> 8 â‰¥ 7`; no `_DAT_0055be*` alias assigned |
| +0x38 | DAT_00580de8 | **sky_layer_array[8]** |
| +0x3C | DAT_00580dec | **sky_layer_array[9]** |
| +0x40 | DAT_00580df0 | **below_angle_scale** — multiplied by `(âˆ’0xC0 âˆ’ angle)` and shifted >>6 for below-horizon layer index; used by `SetActiveLayerByAngle` when `angle < âˆ’0xC0` |
...
| +0x70 | DAT_00580e20 | VA of **palette buffer** (0xc0 Ã— 4 bytes) — copied to working gradient buffer |
...
77 61 76 65 31 2E 53 48 00                       â† "wave1.SH\0" — ocean mesh
...
- New `lib/src/lay.cpp` + `lib/include/fx/lay.h` — parse DLL data header, LAYER array, gradient byte arrays, wave params
- New `cli/cmd_lay.cpp`:
  - `fx lay dump <file.LAY>` — exports atmosphere parameters as JSON
  - `fx lay gradient <file.LAY> -o gradient.png` — renders sky colour ramp as a PNG strip (one pixel per step)
