...
| 25 | `$2000000` | Emplaced AA artillery | Inferred | KS12 (85mm), KS19 (100mm), M1939 (37mm). No entity+0x09 bit test found in FA.EXE full decompile (`DumpAllFunctions.txt`). Only `_gamePrefs & 0x2000000` hits found (not an entity test). |
...
- ~~**ot_flags bit 18 (`$40000`) label**~~ **PARTIALLY RESOLVED (2026-05-19):** Confirmed read from `*(uint*)(entity+1) & 0x40000` in two fuel bar display functions — when set, doubles the fuel capacity denominator for the bar scale. Label updated from "arrestor-wire deck" to "fuel bar capacity doubler / conventional carrier". Whether an additional carrier-landing handler also tests this bit is still unconfirmed.
- ~~**ot_flags bits 19, 20 (`$80000`, `$100000`)**~~ **RESOLVED (2026-05-19):** Both confirmed in `_PROJServiceWeapon@24` (0x4c4700) — bit 19 = high burst-size modifier (25% trigger), bit 20 = medium burst-size modifier (75% trigger). Labels updated; see bit semantics table above.
- ~~**ot_flags bit 26 (`$4000000`)**~~ **RESOLVED (2026-05-19):** Confirmed from `DumpAllFunctions.txt` — `_GVEventProc` (0x473f50) tests `DAT_0050d271 & 0x4000000` (entity+0x09). When set, the post-kill evasion branch (`FUN_00474570` + `_ImmediateService_0`) is skipped. SA-2A does not dodge after a kill hit. Label updated; see bit semantics table above.
- **ot_flags bit 25 (`$2000000`)**: No entity+0x09 bit test found anywhere in FA.EXE full decompile (`DumpAllFunctions.txt`). Only `_gamePrefs & 0x2000000` hits exist (not entity-level tests). Label "Emplaced AA artillery" remains inferred from unit distribution (KS12/KS19/M1939). Terrain overlay DLL not yet imported — possible remaining test site. No further action possible without importing terrain DLL.
