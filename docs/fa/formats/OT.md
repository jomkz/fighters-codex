...
dishes, etc. [BRF](BRF.md) plain text (NOT a Win32 PE DLL); decompressed sizes
...
Plain text; BRF syntax (see [BRF.md](BRF.md) for the token grammar).
...
- The `~` prefix has two distinct uses: (1) **destruction state replacement** —
  `~BNK5.OT` is "Damaged Shelter 5" with a destroyed model (`dbk5.SH`), zeroed
  `dmg_armor`, and bit 5 of `ot_flags` cleared; placed by the engine at the
  same world position after the base object is destroyed. (2)
  **campaign/theater variant** — `~COLTWR.OT` is an enhanced version of COLTWR
  with an added loop sound and bit 22 set, used in a specific campaign. Check
  the `~` prefix section in [architecture.md](../architecture.md) for the full
  list.
...
Full survey of all 170 OT files. Bit positions confirmed by cross-category
comparison.
...
**Formats:** [BRF](BRF.md) — the family grammar and shared OBJ_TYPE reference;
[SH](SH.md) — 3D shapes referenced by `shape`; [NT](NT.md) — the moving-unit
counterpart; [T2](T2.md) — terrain maps that place OT objects; [M](M.md) —
