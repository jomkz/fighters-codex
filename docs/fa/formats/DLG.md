...
`_DialogWhatItem@0` (DialogWhatItem) returns the current value of `dialogItemPtr` — a global pointer set to the last record that passed the hit test during event dispatch. `_TopCenterDialog` (TopCenterDialog) positions the dialog: `screen_x = (screen_w − dialog_w) / 2`, `screen_y = (screen_h − dialog_h) / 3`.
