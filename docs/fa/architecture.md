...
1. `ParseLayerFile` (`0x004b4370`, = `_WRInit@4`) — loads the LAY DLL through the shared `_RMAccess@8` → `RMFindAndLoad` → `_LoadDLL@16` overlay loader (**not** Windows `LoadLibrary` — see Overlay System note above), copies header block to `hdr`, initialises active-layer pointers to the DLL's default LAYER entry, calls `FindNearestColorEntry` (`0x004b3ad0`) for each LAYER entry to populate `colour_entry_ptr`, then loads cloud/sky PIC wildcards.
