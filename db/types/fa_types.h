...
typedef struct STREAMER_DEF STREAMER_DEF; /* objects: wingtip/contrail streamer def   */
typedef struct _DDSURFACEDESC _DDSURFACEDESC; /* renderer: DirectDraw surface desc (ModeCallback) */
typedef struct WORD_POINT3  WORD_POINT3;  /* terrain: 16-bit 3-vector                */
typedef struct WORD_POINT   WORD_POINT;   /* campaign: 16-bit 2D screen point (map)   */
