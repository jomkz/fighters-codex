...
Machine-readable record of the FA.EXE reconstruction program
([epic #209](https://github.com/jomkz/fighters-codex/issues/209)): every recovered
symbol name, the subsystem it belongs to, and the Ghidra-project ground truth it is
checked against. The human-readable story lives in [docs/fa/](../docs/fa/README.md);
this tree is what keeps those docs and the Ghidra project mechanically in sync.
Program progress is tracked in the generated
[reconstruction matrix](../docs/fa/reconstruction.md).
...
## `inventory/` — Ghidra ground truth (exported, committed)

Fact snapshots regenerated by `scripts/ghidra/export_inventory.sh`; never edited by
hand. Committing them is what lets CI check coverage without a Ghidra install.
