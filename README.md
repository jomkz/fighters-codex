...
`fx` CLI, and `fxs` are the validation layer: a working, byte-identical codec
...
- A **graphical editor** `fxs` (Linux and Windows) — interactive codec validation against real game data; live LIB browser, form-based type editors, image import/export, audio waveform playback, mission and cutscene text editing, pilot identity editing, and screenshot preview
...
Pre-built releases are **64-bit Windows binaries** (Windows 7 or later; `fxs` needs OpenGL 3.3, so Vista-era GPUs or newer) and **Linux x64 tarballs** (glibc 2.35+ — Ubuntu 22.04, Fedora 36, or newer equivalents; libstdc++ is statically linked). Starting with v0.5.0 all three components ship for both OSes — `fxs` runs natively on Linux and Windows via SDL3 + OpenGL 3.3 ([Phase 3 of the roadmap](docs/roadmap.md)); earlier Linux releases (v0.4.0) carried `fx` and `fx_lib` only. Everything also builds from source on Linux with GCC or Clang (see [docs/development.md](docs/development.md)).
...
| `fxs-vX.X.X-windows-x64.zip` | Modders — unzip and run `fxs.exe` from anywhere |
...
| `fxs-vX.X.X-linux-x64.tar.gz` (v0.5.0+) | Modders on Linux — untar and run `fxs` from anywhere |
...
- [docs/gui.md](docs/gui.md) — fxs graphical editor feature reference
