...
// CN_INFO is the one struct whose interior db/types/ maps today; its size is
// read from CN_ReadConfig's 0xDDC-byte body. The rest of the vocabulary is
// declared opaque on purpose -- see #454, which is an RE investigation, not a
// parsing job (objects are variable-size, and the field census overlaps).
