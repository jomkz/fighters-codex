...
// The recovered widths, asserted. A scalar that is not the size db/ says it
// is would miscompile every access the port makes through it.
static_assert(sizeof(u8) == 1, "u8 must be 1 byte(s)");
static_assert(sizeof(s8) == 1, "s8 must be 1 byte(s)");
static_assert(sizeof(u16) == 2, "u16 must be 2 byte(s)");
static_assert(sizeof(s16) == 2, "s16 must be 2 byte(s)");
static_assert(sizeof(u32) == 4, "u32 must be 4 byte(s)");
static_assert(sizeof(s32) == 4, "s32 must be 4 byte(s)");
static_assert(sizeof(fixed24) == 4, "fixed24 must be 4 byte(s)");
static_assert(sizeof(undefined1) == 1, "undefined1 must be 1 byte(s)");
static_assert(sizeof(undefined2) == 2, "undefined2 must be 2 byte(s)");
static_assert(sizeof(undefined4) == 4, "undefined4 must be 4 byte(s)");

// The recovered LAYOUTS, asserted (#454). Each size is a claim the code proves:
//   entity   0xDE  -- the common region of every object record. The record itself
//                     is variable-size (0xDE + type->obj_ext_size), so this is the
//                     shared part, not the whole object: OBJAdd and GetCurObj both
//                     compute the total the same way, independently.
//   OBJ_TYPE 0x81  -- the common HEADER of the type record; its total size is
//                     self-declared at +0x01, and the class extension follows.
